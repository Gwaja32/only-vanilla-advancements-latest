...
public class OnlyVanillaAdvancements implements ModInitializer {
        ...
        try {
            ...
            if (cfg.keep_parent_advancements) {
                ...
                while (!stack.isEmpty()) {
                    ...
                    AdvancementNode pa = nodes.get(id);
                    if (pa == null) continue;
                    AdvancementNode parent = pa.parent();
                    if (parent == null) continue;
                    Identifier pid = parent.holder().id();
            ...
            if (!toRemove.isEmpty()) {
                ...
                server.getPlayerList().getPlayers().forEach(p -> p.getAdvancements().reload(loader));
    ...
    private String safePackId(Resource res) {
        try { return res.sourcePackId(); } catch (Throwable t) { return ""; }
    }

    private boolean isEffectiveVanilla(ResourceManager rm, Identifier advId) {
        try {
            String resPath = "advancements/" + advId.getPath() + ".json";
            Identifier resId = Identifier.fromNamespaceAndPath("minecraft", resPath);
